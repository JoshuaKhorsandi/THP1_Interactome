...
path <- paste(urld, "acc=GSE205648", "file=GSE205648_raw_counts_GRCh38.p13_NCBI.tsv.gz", sep="&");
...
# sample selection
gsms <- "XXXXXX558866772233114400" # everything except drugs
# gsms <- "XXXXXXXXXXXXXXXX00223311" # 1hpi, 8hpi, 2hpi, 4hpi
...
gs <- factor(sml)
groups <- make.names(c("WT8hpi","WT2hpi","Uninfected","WT1hpi","WT4hpi","dotA1hpi","dotA4hpi","dotA8hpi","dotA2hpi"))
levels(gs) <- groups
...
head(tT)
write.table(tT, file="1hpi_vs_8hpi_HEK293FcgR.txt", row.names=F, sep="\t", quote = F)

plotDispEsts(ds, main="GSE205648 Dispersion Estimates")

# create histogram plot of p-values
hist(r$padj, breaks=seq(0, 1, length = 21), col = "grey", border = "white", 
     xlab = "", ylab = "", main = "GSE205648 Frequencies of padj-values")
...
par(mar=c(7,4,2,1))
boxplot(dat[,ord], boxwex=0.6, notch=T, main="GSE205648", ylab="lg(norm.counts)", outline=F, las=2, col=gs[ord])
